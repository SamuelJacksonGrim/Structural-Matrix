...
| `analyzer.py` | spec facade, stream tap, windows, full readout | the spec/output contract changes | `analyze_sequence`, `analyze_file`, `analyze_windows`, `measure`, `to_json_safe` |
| `significance.py` | permutation null-model significance | the structuredness metric changes | `structure_significance` |
| `stability.py` | verdict robustness (margin + jackknife) | the stability rubric changes | `verdict_stability` |
